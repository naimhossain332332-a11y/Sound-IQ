...
class SearchEngine:
    """AI-powered audio search engine with optimized performance."""
    ...
    def load_models(self):
        """Load CLAP models (lazy initialization)."""
        ...
        self.model.eval()
        print("✅ CLAP models loaded")
    
    def get_audio_embedding(self, file_path):
        """Compute CLAP embedding for audio file."""
        self.load_models()
        import torch
        import librosa
    ...
    def remove_folder(self, folder_path):
        """Remove folder and its sounds from database."""
        folder_path = os.path.abspath(folder_path)
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        try:
    ...
    def scan_folder(self, folder_path, progress_callback=None):
        """Scan folder and index audio files."""
        folder_path = os.path.abspath(folder_path)
        ...
        if total_to_index == 0:
            if progress_callback:
                progress_callback(total_files, total_files, "Already indexed")
            conn.close()
            return
        
        print(f"Indexing {total_to_index}/{total_files} files...")
    ...
    def get_sounds_in_folder(self, folder_path):
        ...
        try:
            ...
            folder_id = row[0]
            cursor.execute(
                "SELECT path, filename, duration, sample_rate, channels, size FROM sounds WHERE folder_id = ? ORDER BY filename",
                (folder_id,)
            )
    ...
    def search_keyword(self, query):
        ...
        sql = "SELECT path, filename, duration, sample_rate, channels, size FROM sounds WHERE " + " AND ".join(conditions) + " LIMIT 200"
